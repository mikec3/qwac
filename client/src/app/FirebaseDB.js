...
import {getAuth, signInWithEmailAndPassword, createUserWithEmailAndPassword, signOut, connectAuthEmulator} from 'firebase/auth'
    ...
    //if (window.location.hostname.includes('localhost')) { // this stopped working at some point, but at some point it did work!?!?
    const env = process.env.NODE_ENV // when running npm run dev this will return dev, npm start will set it to be 'production'
    if (env == "development"){
    ...
    const createUser = async (username, email, password) => {
        ...
        // setup pending connections holder for this user /Pending Connection Requests/email/Requester_ID=[]
        const setupPendingRequestsResult = await setupPendingRequests(email);
        console.log(setupPendingRequestsResult);
        
        // user created
        return createUserResult.user;
    }

    // setup pending connections holder for user E-mail upon account creation
    // setup empty array at Pending Connection Requests/email/Requester_ID
    const setupPendingRequests = async (email) => {
        const docRef = doc(db, "Pending Connection Requests", email);
    ...
    // get user's connections UID and Username return as a list of objects [{uid: 'jk4fdjl', Username: 'Mikey'}]
    const getCurrentUsersConnections = async (currentUser) => {
        const docRef = doc(db, "Users", currentUser.uid);
        const docSnap = await getDoc(docRef);
        let finalResult = []; // initialize empty array

        if (docSnap.exists()) {
            let result = docSnap.data().Connections;
            console.log(result);
            console.log(currentUser.uid);
            // push current user's uid to front of list
            result.unshift(currentUser.uid.toString());
            // go get usernames for each uid in list (including current user);
            for (let friendUID of result) {
                const docRefUsername = doc(db, "Users", friendUID);
                const docSnapUsername = await getDoc(docRefUsername);

                if (docSnapUsername.exists()) {
                    let friendInfo = {
                            UID: friendUID,
                            Username: docSnapUsername.data().Username
                        }
                    finalResult.unshift(friendInfo);
                } else {
                    console.log('friend Username not found');
                }
            }

            // return final list of uid and username objects
            return finalResult
        } else {
            console.log('getCurrentUsersConnections failed');
        }
    }
    ...
    // now just returns the response text
    ...
    // return a single question text from given date key
    const getQuestionByDate = async (date) => {
        ...
        if (docSnap.exists()) {
            ...
            console.log('getQuestionByDate failed');
        }
    }
